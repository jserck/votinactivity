<template>
     <section class="m-voting-template" v-timeoutLazy="bg_min_src">
          ...
          ></Dialog>
          <section class="u-vote-count" v-if="userTicketCount>0" @click="census(3)">
          ...
          <header class="clearfix">
               <section class="u-vote-counts" v-if="userTicketCount<=0" @click="census(3)">
                    <section class="u-vote-count-box" v-scroll="'rule'">
                         <span>{{userTicketCount}}</span>
                         <p>点击去做任务</p>
                    </section>
               ...
               <section class="g-video-container">
                    ...
                    <section class="g-vote-msg">
                         <span>
                              已有
                              <i>{{ticketCountSlice(personCount)}}</i>人参与加能量，共计领到
                              <i>{{ticketCountSlice(bookCount)}}</i>本书
                         </span>
                    </section>
               </section>
          </header>
...
export default {
     ...
     data() {
          return {
               ...
               timer: null,
               isInter: true,
               startTime: '2019/01/19 10:50',
               endTime: '2019/02/19 10:56',
               isReadHandler: true,
               bannerlist: [
                    {
     ...
     mounted() {
          let self = this;
          window.navigator.connection.onchange = function (val) {
               self.getMyVote(4);
          }
          this.voteInit()
     },
     ...
     methods: {
          isVoted() {
               let isVoted = IsDateBetween(this.startTime, this.endTime);
               if (isVoted) return;
               this.$router.push({
                    path: '/voted',
                    query: {
          ...
          readHandler() {
                ...
                * @name 阅读
               */
               console.log(this.isReadHandler);
               try {
                    console.log('booklist');
                    // 阅读按钮
                    my.postMessage({
                         event: 'navigatorTo',
                         data: { path: `/shelf/${this.userId}`, query: {} }
                    });
               } catch (error) {
                    // this.$toast('没有书单!')
               }
